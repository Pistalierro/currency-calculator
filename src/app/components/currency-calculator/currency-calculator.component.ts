...
export class CurrencyCalculatorComponent implements OnInit {
  ...
  calculate(): void {
    ...
    const fromRate = fromCurrency === 'UAH' ? 1 : this.currencies().find((c) => c.cc === fromCurrency)?.rate || 1;
    const toRate = toCurrency === 'UAH' ? 1 : this.currencies().find((c) => c.cc === toCurrency)?.rate || 1;

    if (amount && fromRate && toRate) {
      const converted = (amount / fromRate) * toRate;
      this.result.set(Number(converted.toFixed(2))); // Округляем до 2 знаков
    } else {
      this.result.set(null); // Если данные некорректные
    }
  }
